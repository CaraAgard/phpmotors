<?php
//model is for vehicle inventory image uploads
...
function getImages() {
    ...
    //$sql = 'SELECT imgId, imgPath, imgName, imgDate FROM images JOIN carclassification WHERE images.imgPath LIKE "%-tn%"';
    $stmt = $db->prepare($sql);
    $stmt->execute();
...
//thumbnail image path from images table based on vehcileId
function getThumbnailImage($invId) {
    $db = phpmotorsConnect();
    $sql = 'SELECT imgPath  FROM images WHERE invId = :invId AND imgName LIKE "%tn.jpg"';
    $stmt = $db->prepare($sql);
    $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
    $stmt->execute();
    $imageArray = $stmt->fetchAll(PDO::FETCH_ASSOC);
